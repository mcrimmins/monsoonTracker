# =========================================================================
# SYNOPTIC DASHBOARD ANIMATION GENERATOR (V3)
# =========================================================================
...
generate_synoptic_frame <- function(test_year, test_month, test_day, 
  ...
  # --- DYNAMIC TITLE ---
  # test_month is assumed to be a string like "08"
...
# You can set this to a single year (e.g., 2020) or a range (e.g., 1991:2020)
target_years  <- 2020:2020 
# Define the months you want to process (e.g., core monsoon: July, Aug, Sept)
target_months <- c("06","07", "08", "09","10") 
base_slice_dir <- "data/processed/4slices"
base_out_dir   <- "output/frames"

# 2. Loop through Years
for (y in target_years) {
  
  # Create a dynamic output directory for the specific year (e.g., output/frames/2020)
  year_out_dir <- file.path(base_out_dir, as.character(y))
  
  # 3. Loop through Months
  for (m in target_months) {
    
    # Locate the daily NetCDF file to see how many days are in it
    nc_file <- list.files(base_slice_dir, pattern = sprintf("single_%d_%s\\.nc$", y, m), full.names = TRUE)[1]
    
    # Skip gracefully if the file for this year/month doesn't exist
    if (is.na(nc_file) || !file.exists(nc_file)) {
      cat(sprintf("Data not found for %d-%s. Skipping to next month...\n", y, m))
      next
    }
    
    # Briefly open the raster to count the layers (assuming 1 layer = 1 day for tcwv_day)
    ...
    # 4. Loop through Days
    for (d in 1:num_days) {
      # tryCatch prevents a single bad day/corrupt slice from crashing the entire batch job
      tryCatch({
        generate_synoptic_frame(
          ...
          out_dir = year_out_dir,  # Directs output to the year-specific folder
          show_trajectory = TRUE,
          tail_length_days = 4
        )
      }, error = function(e) {
...
# =========================================================================
# Install if needed: install.packages(c("gifski", "av"))
library(gifski)
library(av)

# -------------------------------------------------------------------------
# 1. SETUP DIRECTORIES & PARAMETERS
# -------------------------------------------------------------------------
base_frame_dir  <- "output/frames"
output_anim_dir <- "output/animations"

# Determine your preferred output format ("gif" or "mp4")
output_format <- "mp4" 

# Set animation speed 
# For GIF: delay between frames in seconds (e.g., 0.25 = 4 frames per second)
gif_delay <- 0.25 
# For MP4: frames per second (e.g., 4 fps)
mp4_fps   <- 4    

# Ensure the output animation directory exists
if (!dir.exists(output_anim_dir)) {
  dir.create(output_anim_dir, recursive = TRUE)
}

# -------------------------------------------------------------------------
# 2. LOCATE YEAR DIRECTORIES
# -------------------------------------------------------------------------
# Get all subdirectories in the frames folder (e.g., output/frames/2019, 2020)
...
# -------------------------------------------------------------------------
# 3. LOOP THROUGH YEARS AND ANIMATE
# -------------------------------------------------------------------------
for (y_dir in year_dirs) {
  
  # Extract the year name (e.g., "2020")
  year_name <- basename(y_dir)
  
  # List all PNGs in chronological order
  ...
  # --- METHOD A: CREATE MP4 (Recommended for file size and pausing) ---
  if (output_format == "mp4") {
    ...
    av_encode_video(
      ...
      output = out_file, 
      framerate = mp4_fps,
      vfilter = "format=yuv420p" # Ensures compatibility with default media players
    )
    ...
    # 16 inches x 7 inches @ 150 DPI = 2400 x 1050 pixels (Matches your ggsave settings)
    gifski(
      png_files = png_files,
      gif_file = out_file,
      width = 2400,   
...
cat("========================================\n")
 ...
 # list.files(
 #  path = "data/processed/4slices/climo", 
 #  recursive = TRUE, 
 #  full.names = TRUE)
